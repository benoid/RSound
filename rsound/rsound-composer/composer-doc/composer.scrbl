...
@racketblock[
'C 'D 'E 'F 'G 'A 'B 'Rest]
Or any symbol which is comprised of one of the above symbols (except @racket['Rest]) combined with an arbitrary number of occurrences of @racket[\#] or @racket[b] (sharp and flat).  @racket[octave] must be an @racket[integer] between -1 and 7, for midi compatibility.

The @racket[duration] argument must be a @racket[beat-value-procedure?].  The duration is stored as a procedure to delay the computation of the note length in frames until the user supplies a tempo to the scheduler.  The @racket[rsound/composer] module provides many of these beat-value-procedures built-in, such as @racket[whole-beat], @racket[half-beat], @racket[quarter-beat], @racket[eighth-beat], and more.  The @racket[beat-value] type will be discussed in detail later.
}
...
The built in @racket[note] constructor makes rsound-composer scores diffuclt for the eye to read.  As such, there are @racket[note] and @racket[rest] constructors for each different built-in @racket[beat-value].  The above example could be rewritten equivalently:
...
In music notation, the dot is a unary operator which modifies a note's duration by 1.5
...
Returns @racket[#t] if @racket[item] is a @racket[note]. Returns @racket[#f] otherwise.
...
An alternate constructor which creates a note from a midi-number and a beat-value-procedure.  The @racket[note-letter] of notes created this way will be limited to the following symbols:
...
Returns a new note which is the musical @racket[interval] up from the original note. @racket[interval] must be one of the following symbols:
...
Returns a new note which is the musical @racket[interval] down from the original note.  Uses same interval symbols as above.

@section{Rests}
A @racket[rest?] is an instance of type @racket[note] which produces silence for the duration of the given @racket[beat-value].
...
Returns @racket[#t] if the item is a @racket[rest?]. Returns @racket[#f] otherwise.

@defproc[(non-rest-note? [item any/c]) boolean?]
Returns @racket[#t] if the item is a @racket[note], but not a @racket[rest?].  Returns @racket[#f] otherwise. 

The following constructors are implemented as procedures instead of values to make source code smoother to to read when mixing notes and rests.  
...
Beat values are named frame lengths which are produced by a @racket[beat-value-procedure?] and used by rsound-composer to schedule notes. It is usually unnecessary for users to be working directly with instances of @racket[beat-value], but it is useful to know about their structure for implementing new @racket[beat-value-procedures].  

@racket[beat-value-procedures] are @racket[procedure?]s which take an @racket[exact-positive-integer] representing a tempo, and return a @racket[beat-value].  The rsound-composer scheduler takes a @racket[note]'s @racket[beat-value-procedure], and calls it with the tempo argument to figure out the frame-length of the note.  Frame-length calculation is delayed until scheduling time so the user can easily specify a tempo for larger sections of notes at runtime or in a repl.  

@defproc[(beat-value-procedure? [beat-value-proc any/c]) boolean?]
Returns @racket[#t] if the @racket[beat-value-proc] is a @racket[procedure?] which both takes an @racket[exact-positive-integer?] and returns a @racket[beat-value].  @racket[beat-value-procedure?] returns @racket[#f] otherwise.
...
Takes a tempo and produces a whole-beat.  A whole-beat is named as such because it is the beat-value corresponding to a whole note.  To clarify, it does not refer to a single beat in a time signature. All beat-values and beat-value-procedures are named according to thisscheme.
...
Takes a @racket[beat-value-procedure?] and an @racket[exact-positive-integer?] to subdivide by, and a returns a new @racket[beat-value-procedure], which is the subdivision of the original procedure by @racket[subdivisor].
...
The library provides a way to create synthesizer instruments for playing @racket[rsound/composer] elements.

@defstruct[instrument ([name string?] [conversion-proc procedure?])]
An instrument is a structure which contains a name and a note-to-rsound conversion @racket[procedure].  The rsound/composer scheduler uses @racket[instruments] to turn @racket[notes] into @racket[rsounds] before scheduling them on a given @racket[pstream].  Many playback functions allow users to specify an instrument using the @racket[#:instrument] keyword.

@racketblock[
(play-note (half-note 'E 4) #:instrument (vgame-synth-instrument 5))
]

The rsound/composer library provides wrappers around rsound/single-cycle synthesizers.
...
A @racket[struct-measure] is a data type which contains a list of notes.  Since having @racket[list]s strewn throughout a score muddy its legibility, a cleaner constructor is provided:
...
@defproc[(measure-is-valid? [measure measure?] [time-signature time-signature?]) boolean?]
Returns true if the combination of notes in @racket[measure] are valid in relation to the @racket[time-signature].

@defproc[(measure-frames [measure measure?] [tempo exact-positive-integer?]) exact-nonnegative-integer?]
Takes a measure and a tempo and returns the duration of the measure in frames.  This is useful when manually queuing measures on pstreams.
